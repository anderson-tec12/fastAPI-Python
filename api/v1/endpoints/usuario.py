...
async def post_usuario(usuario:UsuarioSchemaCreate, db:AsyncSession = Depends(get_session)):
  ...
  async with db as session:
    try:
      ...
      raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Email em uso")
...
async def put_usuario(usuario_id:int, usuario:UsuarioSchemaUp, db:AsyncSession = Depends(get_session)):
  async with db as session:
    ...
    if usuario_up:
      ...
      if usuario.eh_admin == True:
        usuario_up.eh_admin = True

      if usuario.eh_admin == False:
        usuario_up.eh_admin = False
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db:AsyncSession = Depends(get_session)):
  ...
  if not usuario:
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Fail login!!")
